...
with open(EF.filename) as csvfile:
  ...
  for row in csv_reader:
    ...
    #contact = gsd.ExpSearchContactById(row["matter_ref"])
    ...
    #matter = row["matter_ref"] #this is used when there are no matter numbers.
    user = gsd.ExpSearchU(row["billed_by_user_ref"])
    items = gsd.TimeSearch_items(row["item_code"])
    ...
    if post_data == {}:
      ...
      response = requests.request("POST", url + "timeentries", data=post_data_json, headers=headers)
      reponse_body = json.loads(response.text)
      if response.status_code == 200:
                print("Processing row " + str(line_count) + ": Time Entry successfully uploaded") #posting data and printing the response
                next
